...
void HashTable::Insert(Bid bid) {
    ...
    if (oldNode == nullptr) {
        Node* newNode = new Node(Node(bid, key));
...
void HashTable::PrintAll() {
    ...
    while (node != nullptr) {
        if (node->key != UINT_MAX) {
            cout << "Key: " << node->key << " | " << "Bid Id: " << node->bid.bidId << " | " << "Title: " << node->bid.title << " | "  << "Ammount: $" << node->bid.amount << " | " << "Fund: " << node->bid.fund << " | "
        }
        node = node->next;
        }
    }
